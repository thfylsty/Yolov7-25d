...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
            ...
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    with open(lb_file, 'r') as f:
                        ...
                        l_all = np.array([np.array(x[1:], dtype=np.float32) for x in l])
                        l_all[:, 0:2] /= shape
                        l_all[:, 2:4] /= shape
                        l_xywh = xyxy2xywh(l_all)

                        # if any([len(x) > 8 for x in l]):  # is segment
                        #     classes = np.array([x[0] for x in l], dtype=np.float32)
                        #     segments = [np.array(x[1:], dtype=np.float32).reshape(-1, 2) for x in l]  # (cls, xy1...)
                        # l = np.concatenate((classes.reshape(-1, 1), segments2boxes(segments)), 1)  # (cls, xywh)
                        l = np.concatenate((classes.reshape(-1, 1), l_xywh), 1)  # (cls, xywh)
                        l = np.array(l, dtype=np.float32)
                    if len(l):
                        # assert l.shape[1] == 5, 'labels require 5 columns each'
                        assert l.shape[1] == 12, 'labels require 12 columns each' # 12 columns for v2x 25d
        ...
        x['version'] = 0.2  # cache version, 0.1 ori , 0.2 for v2x 25d
    ...
    def __getitem__(self, index):
        ...
        if nL:
            ...
            labels_out[:, 1:] = torch.from_numpy(np.concatenate((labels[:,:5],labels[:,7:8]),1))
...
def hist_equalize(img, clahe=True, bgr=False):
    ...
    if clahe:
        ...
        yuv[:, :, 0] = cv2.equalizeHist(yuv[:, :, 0])  # equalize Y channannel histogram
