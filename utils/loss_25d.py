...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.deploss = torch.nn.functional.l1_loss
        for k in 'na', 'nc', 'nl', 'anchors':
            setattr(self, k, getattr(det, k))

    def __call__(self, p, targets):  # predictions, targets, model
        device = targets.device
        lcls, lbox, lobj,ldep = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        tcls, tbox,tdep, indices, anchors = self.build_targets(p, targets)  # targets
        ...
        loss = lbox + lobj + lcls + ldep
        return loss * bs, torch.cat((lbox, lobj, lcls,ldep, loss)).detach()

    def build_targets(self, p, targets):
        # Build targets for compute_loss(), input targets(image,class,x,y,w,h)
        na, nt = self.na, targets.shape[0]  # number of anchors, targets
        tcls, tbox,tdep, indices, anch = [], [], [], [],[]
        ...
        return tcls, tbox,tdep, indices, anch
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # Depth
                selected_tdep = targets[i][:, -1]
                d = ps[:, -1].sigmoid()
                d = (d**3)*250 # calculte
                ldep += self.deploss(d,selected_tdep)


                # d = ps[:, -1].sigmoid()
                # ldep += self.deploss(d,1/tdep[i])
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
                ...
                p_cls.append(fg_pred[:, 5:-1])
                p_dep.append(fg_pred[:, -1:])
